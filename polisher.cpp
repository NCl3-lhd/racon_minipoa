...
Polisher::Polisher(std::unique_ptr<bioparser::Parser<Sequence>> sparser,
    ...
    for (uint32_t i = 0; i < num_threads; ++i) {
        ...
        // para->align_mode = ABPOA_GLOBAL_MODE;
        // para->gap_mode = ABPOA_LINEAR_GAP;
...
Polisher::~Polisher() {
    ...
    //
    for (std::size_t i = 0; i < alignment_engines_.size(); ++i) {
        // abpoa_free_para(alignment_engines_[i]);
        delete alignment_engines_[i];
